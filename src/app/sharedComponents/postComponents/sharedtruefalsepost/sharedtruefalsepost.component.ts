import { Component, OnInit, ViewChild, ViewContainerRef, ComponentFactoryResolver, ElementRef, HostListener } from '@angular/core';
...
@Component({
    selector: 'app-sharedtruefalsepost',
    // host: {
    //     '(document:click)': 'handleClick($event)',
    // },
    templateUrl: './sharedtruefalsepost.component.html',
    styleUrls: ['./sharedtruefalsepost.component.scss', './../sharetextpostview/sharetextpostview.component.scss', './../textpost/allpost.css']
})
export class SharedtruefalsepostComponent implements OnInit {
    ...
    hideSocialList() {
        let id = this.u_id + '_' + this.post_id + '_shareTfShareList';
    ...
    handleClick(event) {
        ...
        var inside = false;
        do {
            if (this.postright && clickedComponent === this.postright.nativeElement) {
                inside = true;
            }
    ...
    ngOnInit() {
        ...
        // }
        this.postPublicShareLinkCopy = this._constantService.staticPostShareLink + this.arr.SHARED_POST_DATA['USER_POST_ID'];
        this.postPublicShareLink = this._constantService.staticPostShareLink + this.arr.SHARED_POST_DATA['USER_POST_ID'] + '/' + this.arr['URL'];
        ...
        if (this.arr != null) {
            ...
            if (this.arr['TEXT']) {
                this.post_data = this.postdata.decodeURIPostData(this.arr['TEXT']).trim();
            ...
            if (this.arr['SHARED_POST_DATA']['TEXT']) {
                this.sharedPost_data = this.postdata.decodeURIPostData(this.arr['SHARED_POST_DATA']['TEXT']).trim();
    ...
    ngAfterViewInit() {
        ...
        this.findInterestPos();
        if (this.latestComment != undefined) {
            if (Object.keys(this.latestComment).length != 0) {
        ...
        if (this.responseStatus == 1 && this.questionTime != 0 && this.resultDeclared == false) {
            ...
            if (this.myResponse[0] == "A") {
                (<HTMLInputElement>document.getElementById(this.post_id + "_option1")).checked = true;
            } else if (this.myResponse[0] == "B") {
                (<HTMLInputElement>document.getElementById(this.post_id + "_option2")).checked = true;
            }
        }
