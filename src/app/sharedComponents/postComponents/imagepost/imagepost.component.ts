...
@Component({
    ...
    styleUrls: ['./imagepost.component.scss', './../textpost/allpost.scss']
})
export class ImagepostComponent implements OnInit, AfterViewInit {
    ...
    post_id;
    hideSpan = 1;
    imagepostviewsec: boolean = false;
    show: boolean = false;
    editPostId;
    ...
    hideSocialList() {
        let id = this.u_id + '_' + this.post_id + '_imgShareList';
    ...
    shareimagePostscreen() {
        this.arr['EDIT_POST'] = false;
        this.sharescreenimagesec = true;
        let body = document.getElementsByTagName('body')[0];
        body.classList.add("body-overflow");
    }
    ...
    sharescreenimageparenthide(sharescreenimgpara) {
        this.sharescreenimagesec = false;
        if (sharescreenimgpara) {
            this.share++;
        }
    }

    imgConfirmDelete: boolean = false;

    imgDeletePostCnf() {
        this.imgConfirmDelete = true;
    ...
    imgCloseDeleteConfirm(event) {
        this.imgConfirmDelete = event['closePopUpStatus'];
