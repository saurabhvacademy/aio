...
@Component({
    selector: 'app-sharedsinglechoicepost',
    // host: {
    //     '(document:click)': 'handleClick($event)',
    // },
    templateUrl: './sharedsinglechoicepost.component.html',
    styleUrls: ['./sharedsinglechoicepost.component.scss', './../sharetextpostview/sharetextpostview.component.scss', './../textpost/allpost.css']
})
export class SharedsinglechoicepostComponent implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        //                    this.min = this.questionTime%60;
        //                    this.hour = Math.floor(this.questionTime/60);
        //                    //this.startTimer();
        //                }else{
    ...
    hideSocialList() {
        let id = this.u_id + '_' + this.post_id + '_shareSShareList';
    ...
    ngOnInit() {
        ...
        if (this.arr['PAGE_UUID'] != '' && this.arr['PAGE_UUID'] != null) {
            ...
            this.title = this.postdata.decodeURIPostData(this.arr['TITLE']);
        ...
        if (this.sharedPost_data) {
            //this.sharedPost_data = this.sharedPost_data.replace(/  /, " &#160;");
        }
    ...
    changeView() {
        ...
        //        }
        //        if (this.option4 != "") {
        //            //ocument.getElementById(this.post_id + "_opt4_text").innerHTML = this.option4;
        //            this.option4 = this.postdata.getOption4TextToSave(this.post_id);
        //        }
        //        if (this.option5 != "") {
        //            //document.getElementById(this.post_id + "_opt5_text").innerHTML = this.option5;
    ...
    putUserPostUnsaved() {
        ...
        this._constantService.fetchDataApi(this._constantService.putUserPostUnsaved(),updatePostUnsaved).subscribe(data => {
            ...
            if (status == this._constantService.success_msg) {
                ...
                if (urlArr[1] == "saved") {
                    var count = document.getElementById(this.post_id + "_count");
                    if (count != null) {
                        if (parseInt(count.innerHTML) == 1) {
                            count.style.display = 'none';
                            count.innerHTML = (parseInt(count.innerHTML) - 1).toString();
