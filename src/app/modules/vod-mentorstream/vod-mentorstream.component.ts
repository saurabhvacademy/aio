...
@Component({
  selector: "app-vod-mentorstream",
  templateUrl: "./vod-mentorstream.component.html",
  styleUrls: [
    "./../live-stream-admin/live-stream-admin.component.scss",
    "./../live-stream/live-stream.component.scss",
    "./vod-mentorstream.component.scss",
    "./adminchat.css",
  ],
})
export class VodMentorstreamComponent implements OnInit {
  mentorUUID: any;
  delSocialTyp: any;
      ...
      .subscribe(
        (data) => {
          ...
          if (status == this._constantService.success_msg) {
            ...
            var streamDurationInMilliSeconds =
              responseData.DURATION * 60 * 1000;
            var timeSpent = new Date().getTime() - responseData.START_TIME;
            this.timeLeftMilliseconds =
              streamDurationInMilliSeconds - timeSpent - 330 * 60000;
            this.timeLeft = this._constantService.getDateFromMilliseconds(
              "hh:mm:ss",
              this.timeLeftMilliseconds
            );
            this.startTimer();
            ...
            if (responseData.MENTOR_DATA.PROFILE_PHOTO_PATH)
              userData["profilePic"] =
                responseData.MENTOR_DATA.PROFILE_PHOTO_PATH +
                "profile/" +
                userData["userId"] +
                "_60x60.png";
            else {
  ...
  authPopupClose(typ) {
    ...
    if (typ == 2) {
      ...
      this.checkSocialAuthInterval = setInterval(() => {
        //for getting status after 10 seconds interval
        ++hitCounter;
        this.getSocialAuthStatus(this.usrCode);
        if (hitCounter > 2) {
          //for getting status only for 3 times in 30 seconds
          clearInterval(this.checkSocialAuthInterval);
        }
      }, 10000);
    }
