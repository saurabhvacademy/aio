...
function getChromeExtensionStatus(extensionid) {
    return new Promise(resolve => {
        if (isFirefox) resolve({'status':'2','message':'Not available for Firefox'});
        ...
        image.onload = function() {
            ...
            setTimeout(function() {
                if (chromeMediaSource == 'screen') {
                    resolve({'status':'2','message':'Installed but not enabled'});
                } else resolve({'status':'1','message':'installed and available'});
            }, 2000);
        };
        image.onerror = function() {
            resolve({'status':'2','message':'Not installed'});
        };
    });
